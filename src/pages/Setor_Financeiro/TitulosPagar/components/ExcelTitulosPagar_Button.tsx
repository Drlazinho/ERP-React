...
import { IGetTitulosPagar } from '../titulosPagar.service'
import formatDateTotvs from '@/utils/formatDataTotvs'

type ExcelButtonEstoqueDeProdutosProps = {
  dados?: IGetTitulosPagar | null
}

export default function ExcelTitulosPagar_Button({
  dados,
    ...
    const exportExcelFile = () => {
        ...
        dados?.titulos.map((item) => {
          sheet.addRow({
            ...
            moeda: item.moeda,
            cotacao: 'R$ ' + item.cotacao,
            valor_Orig: 'R$ ' + item.valor_Orig,
            saldo: 'R$ ' + item.saldo,
        ...
        workbook.xlsx.writeBuffer().then(function (data) {
          ...
          anchor.href = url
          anchor.download = `Titulo A Pagar - ${formatDateTotvs(dados?.titulos[0].emissao)}.xlsx`
          anchor.click()
