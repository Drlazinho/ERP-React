...
import { ITituloReceber } from './titulosReceber.service'
...
//   { label: 'Cotação', key: 'cotacao' },
  ...
  { label: 'A Vencer', key: 'avencer' },
  { label: 'A Vencido', key: 'vencido' },
  {
...
const VirtuosoTableComponents: TableComponents<ITituloReceber> = {
  ...
  TableRow: ({ item, ...props }) => (
    <TableRow
      {...props}
      sx={{
        backgroundColor: item?.saldo !== item?.valor_Orig ? 'rgba(255, 0, 0, 0.1)' : 'inherit',
        '&:hover': {
          backgroundColor: item?.saldo !== item?.valor_Orig ? 'rgba(255, 0, 0, 0.15)' : 'rgba(0, 0, 0, 0.04)',
        },
      }}
    />
...
export default function TabelaTitulosReceber({ data, isLoading }: TabelaTitulosProps) {
  const { items, sortConfig, requestSort } = useSortableDataNew<ITituloReceber>(data);

  const fixedHeaderContent = () => (
    <TableRow sx={{ background: '#161e1a' }}>
      {columns.map((col) => (
        <TableCellHead<ITituloReceber>
          key={col.key}
          sortKey={col.key as keyof ITituloReceber}
  ...
  const rowContent = (_index: number, row: ITituloReceber) => (
    <>
      {columns.map((col) => (
        <TableCell
          key={col.key}
  ...
  return (
    ...
    >
      {
        !items &&   <Typography variant='h4' sx={{
          textAlign:'center',
          padding: 4
        }}>Títulos a Receber sem registro no dia</Typography>
      }
      {
