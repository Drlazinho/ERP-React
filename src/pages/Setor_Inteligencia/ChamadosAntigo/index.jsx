...
import TabelaChamados from './tabela/TabelaChamados';
import {
  GetChamados,
  GetChamadosId,
  PutChamados,
} from '@/services/chamados/chamados.service';
import ModalChamadoDetalhes from './modal';
...
import {
  ...
  PostChamadosDetalhe,
  PutChamadosDetalhe,
} from '@/services/chamadosDetalhe/chamadosDetalhe.service';
...
import { formatDateSendApi, formatDateTime } from '@/utils/formatDateInput';
import useUsuarioLocal from '@/hooks/usuarioLocal.hook';
import { PiMicrosoftExcelLogoBold } from 'react-icons/pi';
import { GetChamadosFiltrado } from '@/services/chamados/chamados.service';
...
export default function Chamados() {
  ...
  const [infoChamadosId, setInfoChamadosId] = useState(null);
  ...
  const [userSolicitante, setUserSolicitante] = useState(null);
  ...
  const handleOpen = (value, email, solicitante) => {
    handleGetChamadosId(value);
    setUserSolicitante(solicitante);
    setOpen(true), setEmailSolicitante(email);
  };
  ...
  const handleOpenDet = (id) => {
    setShowModalDet(true), setIdChamadoSelecionado(id);
  };
  ...
  const handleFetch = () => {
    ...
    GetChamadosFiltrado({ EmailUsuario: email })
      .then((res) => {
        setListaChamados(res.value);
      })
      .catch(() =>
        addToast({
          type: 'danger',
  ...
  const loaderDataChamadosDetalhe = (id) => {
    GetChamadosDetalhe({ idChamado: id })
      .then((res) => {
        setListaChamadosDetalhes(res);
      })
      .catch((error) => {
        addToast({
          type: 'danger',
  ...
  const handleGetChamadosId = (id) => {
    GetChamadosId(id)
      .then((res) => {
        setInfoChamadosId(res);
      })
      .catch(() => {
        addToast({
          type: 'danger',
    ...
    loaderDataChamadosDetalhe(id);
  };

  const handleUpdateChamado = (value) => {
    PutChamados(value)
      .then(() => {
        addToast({
          type: 'success',
  ...
  const handleEnviarEmail = useCallback((value) => {
    const data = new Date();
    const destinatario = emailSolicitante;
...
<path d="M169.791 50.1615L169.818 53.0686L170.07 53.0956L170.313 53.1226V50.6295C170.313 49.0815 170.349 48.1635 170.394 48.2085C170.439 48.2625 170.97 49.2255 171.564 50.3685C172.365 51.8985 172.689 52.4386 172.806 52.4386C172.923 52.4386 173.256 51.8805 174.093 50.2785L175.218 48.1185L175.245 50.6205L175.263 53.1226L175.515 53.0956L175.758 53.0686L175.785 50.1615L175.803 47.2635H175.425H175.038L173.922 49.4325L172.815 51.6015L171.681 49.4325L170.547 47.2635H170.16H169.773L169.791 50.1615Z" fill="black"/>
                ...
                <tbody style="width: 100%">
                  <tr style="width: 100%">
                    ...
                    >
                      ...
                      "
                    >
                      <b>Responsável Execução: </b>  ${responsavelExecucao}
                      </p>
  ...
  const handleGetChamadoDet = (id) => {
    GetChamadosDetalheId(id)
      .then((res) => {
        setDetalheSelecionado(res);
      })
  ...
  const handleDeleteChamadoDet = (idDetalhe, idChamado) => {
    DeleteChamadosDetalheId(idDetalhe)
      .then(() => {
        addToast({
          type: 'success',
  ...
  const exportExcelFile = () => {
    ...
    listaChamados.chamados.map((item) => {
      sheet.addRow({
        id: item.id,
        titulo: item.titulo,
        status: item.status,
        ...
        atualizacao: formatDateTime(item.atualizacao),
        abertura: formatDateTime(item.abertura),
        ...
        >
          ...
          >
            ...
            <Typography variant="h4">
              Help Amvox - Gerenciador de Chamados [ANTIGO]
            </Typography>
          </Box>
          <Button
            variant="contained"
        ...
        {/* <Grid container columnSpacing={1} rowSpacing={1} sx={{ p: 1, pt: 0 }}>
          <Grid item xs={6} sm={4}>
            <NumberBox3
              ...
              monthValueOrDefault={listaChamados.qtdAberto}
              color={'#001E3F'}
              status={1}
              fontColor={'#fff'}
              iconSetor={'inteligencia'}
              setor={`${listaChamados.chamados?.[0].setor}`}
              noVariantCurrent
              noLink
              toLink={''}
          ...
          <Grid item xs={6} sm={4}>
            <NumberBox3
              ...
              monthValueOrDefault={listaChamados.qtdFechado}
              color={'#001E3F'}
              status={1}
              fontColor={'#fff'}
              iconSetor={'inteligencia'}
          ...
          <Grid item xs={12} sm={4}>
            <NumberBox3
              ...
              monthValueOrDefault={listaChamados.qtdTotal}
              color={'#001E3F'}
              status={1}
              fontColor={'#fff'}
              iconSetor={'inteligencia'}
