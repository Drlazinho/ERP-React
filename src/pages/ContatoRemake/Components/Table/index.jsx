...
export default function TableMeusChamados({ dadosTabela = [], upDate }) {
  ...
  const dadosOrdenados = dados.sort(
    (a, b) => new Date(b.dataAtualizacao) - new Date(a.dataAtualizacao)
  );
      ...
      <Table sx={{ minWidth: 500 }} aria-label="custom pagination table">
        <TableHead>
          <TableRow>
            ...
            <TableCell sx={{ borderBottom: 'none' }}>Ocorrências</TableCell>
        ...
        <TableBody>
          {(rowsPerPage > 0
            ...
            <TableRow key={row.id}>
              <TableCell sx={{ borderBottom: 'none' }}>
                <ModalDetalhes data={row} upDate={upDate} />
              </TableCell>
              <TableCell sx={{ borderBottom: 'none' }}>{row.id}</TableCell>
              <TableCell sx={{ borderBottom: 'none' }}>{row.titulo}</TableCell>
              <TableCell sx={{ borderBottom: 'none' }}>
                {row.categoria || 'Sem info'}
              </TableCell>
              <TableCell
                sx={{
                  borderBottom: 'none',
                  color:
                    (row.categoria === 'APOIO AO USUARIO' && row.dias > 1) ||
